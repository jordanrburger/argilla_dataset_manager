...
def get_argilla_client() -> rg.init:
    """
    ...
    Returns:
        rg.init: Configured Argilla client
...
def get_or_create_workspace(
    client: rg.init, workspace_name: str, create_if_missing: bool = True
    ...
    try:
        ...
        logger.error(f"Error accessing workspace: {str(e)}")
        return None
...
def get_or_create_dataset(
    ...
    """
    try:
        dataset = client.datasets(name=dataset_name, workspace=workspace.name)
        if dataset is not None:
            return dataset
        if create_if_missing:
            logger.info(f"Creating dataset: {dataset_name}")
            dataset = rg.Dataset(name=dataset_name, workspace=workspace.name, settings=settings, client=client)
