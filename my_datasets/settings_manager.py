...
class SettingsManager:
    ...
    def create_settings(self, settings: DatasetSettings) -> Dict[str, Any]:
        ...
        for field in settings.fields:
            field_def = {"type": field.field_type, "required": field.required}
            if field.choices:
    ...
    def load_settings(self, filename: str) -> DatasetSettings:
        ...
        fields = [FieldSettings(**field_data) for field_data in data["fields"]]
    ...
    def list_available_settings(self) -> List[str]:
        ...
        """
        return [f.stem for f in self.settings_dir.glob("*.yaml")]


# Example settings templates
def create_qa_dataset_settings(
    name: str, include_context: bool = True, include_keywords: bool = True
) -> DatasetSettings:
    """
    ...
    fields = [
        FieldSettings(
            name="prompt", field_type="text", required=True, description="The question or prompt"
        ),
        FieldSettings(
            name="response", field_type="text", required=True, description="The answer or response"
        ),
    ]
    ...
    if include_keywords:
        fields.append(
            FieldSettings(
                name="keywords", field_type="text", required=False, description="Keywords or tags"
            )
        )
